...
cpu_model = load_model(name="capsule_model_0")
function full_model(x_raw)
    x = (x_raw .- μ) ./ (1e-5 .+ σ)
    ŷ = cpu_model(x)
    return ŷ
end
...
function newton_solve!(solver, parameters, previous_variables, mask;
    ...
    variables[idx_duals[.!mask]] .= 0.0*1e-5 # inactive constraints duals = 0.0 slacks >= 0.0
    variables[idx_slacks[mask]] .= 0.0*1e-5 # active constraints duals >= 0.0 slacks = 0.0
...
function active_set_step!(mechanism::Mechanism, z0, u, storage, i, model)
    set_current_state!(mechanism, z0)
    set_input!(mechanism, u)
    update_parameters!(mechanism)

    parameters = deepcopy(mechanism.solver.parameters)
    previous_variables = deepcopy(storage.variables[i])

    ############################################################################
    xi_raw, yi = extract_feature_label(mechanism, storage, i)
    # xi = (xi_raw .- μ) ./ (1e-5 .+ σ)
    # ŷi = cpu_model(xi)
    ŷi = model(xi)
    guessed_mask = ŷi .> 0.50
    ############################################################################

    variables, success, mask = active_set_solve!(
        mechanism.solver, parameters, previous_variables, predicted_mask;
        max_active_iterations=2,
        max_iterations=20,
        line_search_iterations=5,
        single_jacobian=false,
...
xi_raw, yi = full_feature_extraction(mech, storage_test, i0)
xi = (xi_raw .- μ0) ./ (1e-5 .+ σ0)
ŷi = cpu_model(xi)
...
sum(abs.(true_mask - guessed_mask))
...
norm(opt_vars[1:num_primals] - current_variables[1:num_primals])
