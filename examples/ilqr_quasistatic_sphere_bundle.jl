...
xp2_start = [
    ...
    +0.2425349448513732,
    -0.2449909347641345]
xc2_start = [+0.75,0.20,0.0]
xp2_goal = [
    -0.5,
    +0.2425349448513732,
    -0.2449909347641345]
xc2_goal = [+0.15,0.20,0.0]
xp2 = [
    -0.0,
    +0.2425349448513732,
    -0.2449909347641345]
...
u_hover = [0; -0*gravity; 0; 0; -0*gravity; 0]
...
################################################################################
zref = [[xp2_goal; xc2_goal] for i=1:20]
zref = [[xp2_goal; xc2_goal] for i=1:20]
# ## horizon
T = length(zref)

set_mechanism!(vis, mech, [xp2_start; xc2_start])
set_mechanism!(vis, mech, [xp2_goal; xc2_goal])
...
function contt(z, u)
    [
        ...
        1e-2 * (ul - u[1:nu_infeasible]);
        1e-2 * (u[1:nu_infeasible] - uu);
    ]
end
...
# RobotVisualizer.convert_frames_to_video_and_gif("trajopt_sphere_bundle")
