...
struct BilevelContact2D40{T,D,NP,NC} <: BilevelContact{T,D,NP,NC}
    name::Symbol
    parent_name::Symbol
    child_name::Symbol
    index::NodeIndices
...
struct EnvBilevelContact2D40{T,D,NP,NC} <: BilevelContact{T,D,NP,NC}
    name::Symbol
    parent_name::Symbol
    index::NodeIndices
    parent_shape::Shape{T}
...
function BilevelContact2D40(parent_body::AbstractBody{T}, child_body::AbstractBody{T};
    ...
    return BilevelContact2D40{T,D,Np,Nc}(
        name,
        parent_name,
        child_name,
        index,
...
function EnvBilevelContact2D40(parent_body::AbstractBody{T}, child_shape::Shape{T};
        parent_shape_id::Int=1,
        name::Symbol=:contact,
        complementarity_tolerance=1e-4,
        friction_coefficient=0.2) where {T}
    ...
    Np = constraint_dimension(parent_shape)
    Nc = constraint_dimension(child_shape)
    return EnvBilevelContact2D40{T,D,Np,Nc}(
        name,
        parent_name,
        index,
        parent_shape,
...
function residual!(e, x, θ, contact::BilevelContact2D40{T,D,NP,NC},
...
function residual!(e, x, θ, contact::EnvBilevelContact2D40{T,D,NP},
...
function residual!(e, x, θ, contact::BilevelContact2D40, bodies::Vector)
...
function residual!(e, x, θ, contact::EnvBilevelContact2D40, bodies::Vector)
    pbody = find_body(bodies, contact.parent_name)
