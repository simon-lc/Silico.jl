...
poses_noisy = [x[1] .+ 0.05 * [2,1,3] for x in storage.x]
poses_noisy = [x[1] .+ 0.1 * (rand(3) .- 0.5) for x in storage.x]
x_noisy = [p[1:2] for p in poses_noisy] # position
θ_noisy = [p[3] for p in poses_noisy] # orientation


bRw = [[cos(θ[i]) sin(θ[i]); -sin(θ[i]) cos(θ[i])] for i=1:H0]
bRw_noisy = [[cos(θ_noisy[i]) sin(θ_noisy[i]); -sin(θ_noisy[i]) cos(θ_noisy[i])] for i=1:H0]
...
v_noisy, e_noisy = noise_transform(v, e, nβ, poses, poses_noisy)

α_hit = α[hit_indices]
αmax_hit = αmax[hit_indices]
v_hit = v[:,hit_indices]
e_hit = e[:,hit_indices]
v_noisy_hit = v_noisy[:,hit_indices]
e_noisy_hit = e_noisy[:,hit_indices]
...
d0b_noisy = [e_noisy[:,(i-1)*nβ .+ (1:nβ)] + α[(i-1)*nβ .+ (1:nβ)]' .* v_noisy[:,(i-1)*nβ .+ (1:nβ)] for i = 1:H0]
d0w = [x[i] .+ bRw[i]' * d0b[i] for i = 1:H0]
d0w_noisy = [x_noisy[i] .+ bRw_noisy[i]' * d0b_noisy[i] for i = 1:H0]


set_2d_point_cloud!(vis, eye_positions, d0b_noisy; name=:point_cloud)
set_2d_point_cloud!(vis, eye_positions, d0b; name=:point_cloud)
set_2d_point_cloud!(vis, eye_positions, d0w_noisy; name=:point_cloud)
...
d0_noisy_hit = e_noisy_hit .+ α_hit' .* v_noisy_hit
# θinit, kmres = parameter_initialization(d0_hit, polytope_dimensions)
θinit, kmres = parameter_initialization(d0_noisy_hit, polytope_dimensions)
...
visualize_kmeans!(vis, θinit, polytope_dimensions, d0_noisy_hit, kmres)
